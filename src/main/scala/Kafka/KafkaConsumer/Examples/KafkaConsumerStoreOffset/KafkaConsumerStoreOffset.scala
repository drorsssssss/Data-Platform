...
import Kafka.KafkaConsumer.Examples.KafkaConsumerStoreOffset.dal.Tables._
  ...
  def consume(topics:List[String])= {
    ...
    try {
      ...
      while (true) {
        ...
        consumer.seek(new TopicPartition(topics(0),0),0)
        consumer.seek(new TopicPartition(topics(0),1),0)
        consumer.seek(new TopicPartition(topics(0),2),0)
